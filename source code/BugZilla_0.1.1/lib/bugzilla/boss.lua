...
function Boss.Spawn(self, boss_type)
  ...
  if boss_entity == nil then
    boss_entity = game.surfaces['nauvis'].create_entity{
      ...
      position = self:CreateSpawnPosition(boss_type, game.forces.enemy)
    }
...
function Boss.OnEntityDied(_, event)
  local boss_entity = global.BZ_boss.entity
  if boss_entity and boss_entity.valid and event.entity == boss_entity then
    global.BZ_boss.entity = nil
